import { addDays, isBefore, isWithinInterval, isSameDay as dateFnsIsSameDay, parseISO } from "date-fns";
...
export function isWithinRange(date: Date | string, from: Date | string, to: Date | string): boolean {
  const parsedDate = typeof date === "string" ? parseISO(date) : date;
  const parsedFrom = typeof from === "string" ? parseISO(from) : from;
  const parsedTo = typeof to === "string" ? parseISO(to) : to;
  return isWithinInterval(parsedDate, { start: parsedFrom, end: parsedTo });
}
...
export function isSameDay(date: Date | string, compareDate: Date | string): boolean {
  const parsedDate = typeof date === "string" ? parseISO(date) : date;
  const parsedCompareDate = typeof compareDate === "string" ? parseISO(compareDate) : compareDate;
  return dateFnsIsSameDay(parsedDate, parsedCompareDate); 
}
